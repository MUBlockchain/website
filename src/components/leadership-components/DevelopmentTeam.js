...
const DevelopmentTeamRoll = (props) => {
    const {data} = props
    const {edges: posts} = data.allMarkdownRemark

    const createDevRows = posts => {
        let postArray = []
    ...
    return (
        <div className="columns is-multiline">
            {posts && createDevRows(posts)}
        </div>
    )

}

DevelopmentTeamRoll.propTypes = {
...
export default () => (
    <StaticQuery
        query={graphql`
      query DevelopmentTeamQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] },
          filter: {
          fileAbsolutePath: {regex: "/markdown/leadership/"  } ,
          frontmatter: {relation: {eq: "development" }}
          }
        ) {
          edges {
            node {
    ...
    `}
        render={(data, count) => <DevelopmentTeamRoll data={data} count={count}/>}
    />
)